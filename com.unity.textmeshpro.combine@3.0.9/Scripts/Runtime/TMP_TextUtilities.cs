...
namespace TMPro
{
    ...
    public static class TMP_TextUtilities
    {
        ...
        public static void SetXY(this Vector4[] target, int index, Vector2 value)
        {
            target[index].x = value.x;
            target[index].y = value.y;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void SetXYZW(this Vector4[] target, int index, Vector2 value, float z, float w)
        {
            target[index].x = value.x;
            target[index].y = value.y;
            target[index].z = z;
        ...
        public static Texture GetTextureEx(this Material target, int propId)
        {
            if (target != null && target.HasTexture(propId))
            {
                return target.GetTexture(propId);
            }
            else
            {
                return null;
        ...
        public static int GetTextureInstanceID(this Material target, int propId)
        {
            if (target != null && target.HasTexture(propId))
            {
                var tex = target.GetTexture(propId);
                if (tex != null)
                    return tex.GetInstanceID();
                else
                    return 0;
            }
            else
