...
namespace TMPro.EditorUtilities
{
    ...
    public abstract class TMP_BaseShaderGUI : ShaderGUI
    {
        ...
        protected void DoColor(string name, string label, bool hdr = false)
        {
            MaterialProperty property = BeginProperty(name);
            s_TempLabel.text = label;
            Color value = EditorGUI.ColorField(EditorGUILayout.GetControlRect(), s_TempLabel, property.colorValue, false, true, hdr);
            if (EndProperty())
            {
