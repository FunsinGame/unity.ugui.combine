...
namespace UnityEngine.UI
{
    ...
    public class GraphicRegistry
    {
        ...
        public static void RegisterGraphicForCanvas(Canvas c, Graphic graphic)
        {
            ...
            if (CombineRenderManager.IsOpen && !c.additionalShaderChannels.HasFlag(AdditionalCanvasShaderChannels.TexCoord2))
            {
                c.additionalShaderChannels |= AdditionalCanvasShaderChannels.TexCoord2;
            }
        ...
        public static void UnregisterGraphicForCanvas(Canvas c, Graphic graphic)
        {
            ...
            if (instance.m_Graphics.TryGetValue(c, out graphics))
            {
                ...
                if (graphic.CombineMaterialIndex.IsValid())
                {
                    graphic.FreeCombine();
                }
        ...
        internal void MarkAllGraphicsDirty()
        {
            foreach (var item in m_Graphics)
            {
                for (var i = 0; i < item.Value.Count; i++)
                {
                    var graphic = item.Value[i];
                    if (graphic != null)
                    {
        ...
        internal void MarkGraphicsDirtyByCanvas(Canvas canvas)
        {
            if (m_Graphics.TryGetValue(canvas, out var graphics))
            {
